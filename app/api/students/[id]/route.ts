...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        if (error instanceof Error) {
            return NextResponse.json({ code: 'E_SERVER_ERROR', message: 'Error buscando el estudiante', details: error.message }, { status: 500 });
        } else {
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        if (error instanceof Error) {
            return NextResponse.json({ code: 'E_SERVER_ERROR', message: 'Error actualizando el estudiante', details: error.message }, { status: 500 });
        } else {
            return NextResponse.json(error, { status: 500 });
        }
    }
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        if (error instanceof Error) {
            return NextResponse.json({ code: 'E_SERVER_ERROR', message: 'Error eliminando el estudiante', details: error.message }, { status: 500 });
        } else {
            return NextResponse.json(error, { status: 500 });
        }
    }

...
export async function POST(request: Request) {
    try {
        ...
        if (!session || !session.user) {
            return NextResponse.json({ code: 'E_UNAUTHORIZED', error: 'No autorizado' }, { status: 401 });
        }

        if (!Array.isArray(studentsData) || studentsData.length === 0) {
            return NextResponse.json({ code: 'E_INVALID_INPUT', error: 'Se requiere un arreglo de estudiantes' }, { status: 400 });
        }
        ...
        studentsData.forEach((student, index) => {
            const code = `${currentYear}-${(startCodeNumber + index).toString().padStart(4, '0')}`;
        ...
        await Prisma.$transaction(async (prisma) => {
            for (let index = 0; index < studentsData.length; index++) {
                ...
                try {
                    ...
                    const branchId = student.branchId || session?.user?.mainBranch?.id || session?.user?.branches?.[0]?.id;
                    ...
                    const created = await prisma.student.create({
                        data: {
                            ...
                            firstName: student.nombres.trim(),
                            lastName: student.apellidos.trim(),
